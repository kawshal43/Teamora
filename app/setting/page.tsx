    ...
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              ...
              <span className="text-[11px] text-slate-400 italic">Managed by HR</span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <p className="text-slate-400 font-medium">Employee ID</p>
                <p className="font-bold text-slate-900 mt-0.5">{currentUser.employee_id}</p>
              </div>

              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <p className="text-slate-400 font-medium">Department</p>
                <p className="font-bold text-slate-900 mt-0.5">{currentUser.department_name}</p>
              </div>

              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <p className="text-slate-400 font-medium">Designation</p>
                <p className="font-bold text-slate-900 mt-0.5">{currentUser.designation}</p>
              </div>

              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <p className="text-slate-400 font-medium">Role</p>
                <p className="font-bold text-blue-600 mt-0.5 capitalize">{currentUser.role.replace("_", " ")}</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6">
            ...
            <form onSubmit={handleProfileSave} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Bio / Profile Headline</label>
                <textarea
                  rows={2}
                  value={bio}
        ...
        <Card className="p-6 max-w-xl space-y-4 text-xs">
          ...
          <div className="space-y-3">
            {[
              ...
              <label key={i} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50/60 border border-slate-100 cursor-pointer hover:bg-slate-50">
                <input
                  type="checkbox"
                  defaultChecked
      ...
      >
        <div className="space-y-4 text-center">
          <div className="w-36 h-36 mx-auto rounded-full ring-4 ring-blue-100 overflow-hidden shadow-inner relative bg-slate-100">
            <img
              src={currentUser.avatar_url}
              alt="Preview"
